...
func (h *UserHandler) List(c *gin.Context) {
	...
	for _, u := range users {
		...
		if u.DeptID != nil {
			...
			if err := h.db.First(&dept, *u.DeptID).Error; err == nil {
				item.DeptName = dept.Name
			}
		}
		if u.RoleID != nil {
			var role model.Role
			if err := h.db.First(&role, *u.RoleID).Error; err == nil {
...
func (h *UserHandler) Update(c *gin.Context) {
	...
	result := h.db.Model(&model.User{}).Where("id = ?", id).Updates(map[string]interface{}{
		"nickname": req.Nickname,
		"email":    req.Email,
		"phone":    req.Phone,
		"avatar":   req.Avatar,
