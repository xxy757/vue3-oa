    ...
    <n-card title="发起申请">
      ...
      >
        ...
        <n-form-item label="申请标题" path="title">
          <n-input v-model:value="formData.title" placeholder="请输入申请标题" maxlength="50" show-count />
        </n-form-item>

        <!-- 请假申请表单 -->
        <template v-if="formData.type === 'leave'">
          <n-form-item label="请假类型" path="leaveType">
            <n-select v-model:value="formData.leaveType" :options="leaveTypeOptions" placeholder="请选择请假类型" />
          </n-form-item>
          <n-form-item label="开始日期" path="startDate">
            <n-date-picker v-model:value="dateRange.start" type="date" placeholder="请选择开始日期" style="width: 100%" />
          </n-form-item>
          <n-form-item label="结束日期" path="endDate">
            <n-date-picker v-model:value="dateRange.end" type="date" placeholder="请选择结束日期" style="width: 100%" />
          </n-form-item>
          <n-form-item label="请假天数">
            <n-input-number v-model:value="formData.days" :min="0.5" :max="30" :step="0.5" disabled />
          </n-form-item>
        ...
        <template v-if="formData.type === 'expense'">
          <n-form-item label="报销类型" path="expenseType">
            <n-select v-model:value="formData.expenseType" :options="expenseTypeOptions" placeholder="请选择报销类型" />
          </n-form-item>
          <n-form-item label="报销金额" path="amount">
            <n-input-number v-model:value="formData.amount" :min="0" :precision="2" placeholder="请输入报销金额" style="width: 100%">
        ...
        <template v-if="formData.type === 'overtime'">
          <n-form-item label="加班日期" path="overtimeDate">
            <n-date-picker v-model:value="dateRange.overtime" type="date" placeholder="请选择加班日期" style="width: 100%" />
          </n-form-item>
          <n-form-item label="开始时间" path="startTime">
            <n-time-picker v-model:value="timeRange.start" format="HH:mm" placeholder="请选择开始时间" style="width: 100%" />
          </n-form-item>
          <n-form-item label="结束时间" path="endTime">
            <n-time-picker v-model:value="timeRange.end" format="HH:mm" placeholder="请选择结束时间" style="width: 100%" />
          </n-form-item>
          <n-form-item label="加班时长">
            <n-input-number v-model:value="formData.hours" :min="0.5" :max="24" :step="0.5" disabled />
        ...
        <template v-if="formData.type === 'travel'">
          <n-form-item label="目的地" path="destination">
            <n-input v-model:value="formData.destination" placeholder="请输入目的地" maxlength="50" />
          </n-form-item>
          <n-form-item label="开始日期" path="startDate">
            <n-date-picker v-model:value="dateRange.start" type="date" placeholder="请选择开始日期" style="width: 100%" />
          </n-form-item>
          ...
          <n-form-item label="预算金额" path="budget">
            <n-input-number v-model:value="formData.budget" :min="0" :precision="2" placeholder="请输入预算金额" style="width: 100%">
              <template #prefix>¥</template>
            </n-input-number>
          </n-form-item>
        </template>
