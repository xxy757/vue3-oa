<template>
  <div class="dashboard">
    ...
    <n-grid :x-gap="16" :y-gap="16" :cols="4" item-responsive responsive="screen">
      ...
      <n-gi span="4 m:2 l:1">
        <n-card class="stat-card stat-card--schedule" hoverable @click="router.push('/schedule/calendar')">
          <div class="stat-content">
            <div class="stat-icon">
      ...
      <n-gi span="4 m:2 l:1">
        <n-card class="stat-card stat-card--apply" hoverable @click="router.push('/approval/my-apply')">
          <div class="stat-content">
            <div class="stat-icon">
    ...
    <!-- 主要内容区域 -->
    <n-grid :x-gap="16" :y-gap="16" :cols="3" item-responsive responsive="screen" class="main-content">
      <!-- 待办事项 -->
      <n-gi span="3 l:2">
        <n-card title="待办审批" :bordered="false">
          ...
          <n-spin :show="pendingLoading">
            ...
            <n-list v-else hoverable clickable>
              <n-list-item v-for="item in pendingList" :key="item.id" @click="goToApprovalDetail(item.id)">
                <template #prefix>
          ...
          <n-spin :show="scheduleLoading">
            ...
            <n-timeline v-else>
              <n-timeline-item
                ...
                :line-type="schedule.priority === 'high' ? 'dashed' : 'solid'"
              >
                <template #default>
                  <n-text depth="3">
    ...
    <!-- 图表区域 -->
    <n-grid :x-gap="16" :y-gap="16" :cols="2" item-responsive responsive="screen" class="chart-section">
      <n-gi span="2 l:1">
...
import { ApprovalTypeLabels, ApprovalStatusLabels, type ApprovalType } from '@/types/approval'
...
// 获取状态类型
function getStatusType(status: string): 'default' | 'primary' | 'info' | 'success' | 'warning' | 'error' {
...
// 获取优先级类型
function getPriorityType(priority: string): 'default' | 'primary' | 'info' | 'success' | 'warning' | 'error' {
  const map: Record<string, 'default' | 'primary' | 'info' | 'success' | 'warning' | 'error'> = {
    low: 'info',
    medium: 'warning',
    high: 'error'
...
// 格式化日程时间
function formatScheduleTime(schedule: { startDate: string; startTime: string; endTime: string }): string {
...
// 提取审批信息
function extractApprovalInfo(approval: { id: number; type: ApprovalType; title: string; applicantName: string; status: string; createTime: string }) {
  return {
    id: approval.id,
    type: approval.type,
